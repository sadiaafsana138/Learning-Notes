...
### 📊 প্রথম Octet-এর পূর্ণ মানচিত্র ⭐⭐⭐

0 থেকে 255 — কোন সংখ্যা কার, একসাথে দেখো:

| First Octet | Class / উদ্দেশ্য |
|---|---|
| **1 – 126** | **Class A** |
| **127** | **সংরক্ষিত — Loopback / Localhost** ⚠️ |
| **128 – 191** | **Class B** |
| **192 – 223** | **Class C** |
| **224 – 239** | **Class D (Multicast)** |
| **240 – 255** | **Class E (Experimental)** |

⭐ **127-কে আলাদা সারিতে দেখলেই বোঝা যায় Class A কেন 126-এ থামে** — 127 কোনো class নয়, এটা কেটে নেওয়া হয়েছে loopback-এর জন্য।

### Class-এর বিস্তারিত

| Class | প্রথম Octet | **Network** | **Host** | Default Mask | CIDR | বাস্তব ব্যবহার |
|---|---|---|---|---|---|---|
| **A** | **1 – 126** | **১ octet** | **৩ octet** | 255.0.0.0 | **/8** | বিশাল প্রতিষ্ঠান |
| **B** | **128 – 191** | **২ octet** | **২ octet** | 255.255.0.0 | **/16** | মাঝারি প্রতিষ্ঠান |
| **C** | **192 – 223** | **৩ octet** | **১ octet** | 255.255.255.0 | **/24** | বাসা, অফিস, বিশ্ববিদ্যালয় LAN |
| **D** | **224 – 239** | — | — | — | — | **Multicast** |
| **E** | **240 – 255** | — | — | — | — | **Experimental** |

### ছবিতে ভাগটা দেখো ⭐
```
Class A :  N . H . H . H      ১টা network, বিশাল host
Class B :  N . N . H . H      ভারসাম্য
Class C :  N . N . N . H      অনেক network, কম host

           N = Network অংশ     H = Host অংশ
```

**প্রতি network-এ কতগুলো host?**
| Class | হিসাব | Usable Host |
|---|---|---|
| **A** | 2²⁴ − 2 | **16,777,214** |
| **B** | 2¹⁶ − 2 | **65,534** |
| **C** | 2⁸ − 2 | **254** |

⭐ **যত বেশি Network bit, তত কম Host** — এটাই A→B→C-এর মূল ধারা
...
| **169.254.0.1 – 169.254.255.254** | **APIPA** (নিচে বিস্তারিত) ⭐ |
...
### Definition (English)
**APIPA (Automatic Private IP Addressing)** is a Windows feature that allows a host to **automatically assign itself** an IP address from the range **169.254.0.1 – 169.254.255.254** when a **DHCP server is unreachable**.

⭐ **Keywords:** Automatic · Self-assigned · **169.254.x.x** · DHCP ব্যর্থ · Link-local

### Concept (বাংলায়)
সাধারণত তোমার কম্পিউটার **DHCP server** থেকে IP নেয় (DORA প্রক্রিয়ায়)। কিন্তু DHCP server যদি বন্ধ থাকে বা তার সাথে যোগাযোগ না হয় — তখন কম্পিউটার IP-বিহীন হয়ে পড়বে, কোনো যোগাযোগই করতে পারবে না।

**APIPA এই পরিস্থিতির জরুরি ব্যবস্থা** — কম্পিউটার নিজেই একটা IP বসিয়ে নেয়, যাতে অন্তত **local network-এর ভিতরে** কাজ চালানো যায়।

### কীভাবে কাজ করে — ধাপ
```
১. Boot করার পর কম্পিউটার DHCP Discover পাঠায়
২. কোনো DHCP server সাড়া দেয় না (timeout)
৩. কম্পিউটার নিজে 169.254.x.x থেকে একটা random IP বাছে
৪. ARP পাঠিয়ে দেখে ঐ IP অন্য কেউ নিয়েছে কিনা
   → নিয়ে থাকলে আরেকটা বাছে
৫. IP বসিয়ে নেয়, Subnet Mask = 255.255.0.0 (/16)
৬. প্রতি ৫ মিনিট পরপর আবার DHCP খোঁজে
   → DHCP ফিরে এলে APIPA ছেড়ে আসল IP নেয়
```

### Key Points ⭐
1. **Range: 169.254.0.1 – 169.254.255.254** · **Mask: 255.255.0.0 (/16)**
2. **শুধু local network-এ কাজ করে** — **Internet-এ যাওয়া যায় না** ⚠️
3. **Gateway ও DNS বসে না** — এজন্যই ইন্টারনেট চলে না
4. **Link-local address** নামেও পরিচিত (IPv6-এ সমতুল্য: **FE80::/10**)
5. Windows-এ default চালু; Linux/macOS-এও link-local হিসেবে আছে

### 🔧 বাস্তবে কী বোঝায় ⭐⭐⭐

> **`ipconfig` চালিয়ে যদি দেখো IP = `169.254.x.x` — তার মানে DHCP server-এর সাথে যোগাযোগ হয়নি।**

এটি একটি **রোগনির্ণয়ের সংকেত** (diagnostic signal)। সম্ভাব্য কারণ:
- Router/DHCP server বন্ধ
- Ethernet cable খোলা বা নষ্ট
- Wi-Fi সংযোগ আছে কিন্তু DHCP সাড়া দিচ্ছে না
- DHCP-র IP pool শেষ হয়ে গেছে

**সমাধান:** cable ও router দেখো → `ipconfig /release` তারপর `ipconfig /renew`

### ⚠️ গুলিয়ে ফেলো না

| | **APIPA (169.254.x.x)** | **Private IP (192.168.x.x)** |
|---|---|---|
| কে দেয় | **কম্পিউটার নিজে** | **DHCP server / Admin** |
| কখন | **DHCP ব্যর্থ হলে** | স্বাভাবিক অবস্থায় |
| Internet | ❌ **যায় না** | ✅ NAT দিয়ে যায় |
| Gateway | ❌ নেই | ✅ আছে |
| অর্থ | **সমস্যা আছে** ⚠️ | সব ঠিক আছে ✅ |

### 🔁 Revision Box
**APIPA = Automatic Private IP Addressing।** DHCP server না পাওয়া গেলে কম্পিউটার **নিজেই** **169.254.0.1 – 169.254.255.254** থেকে IP নেয়, mask **255.255.0.0**। শুধু **local network**-এ চলে, **Internet-এ যায় না** (gateway/DNS নেই)। `ipconfig`-এ 169.254 দেখা মানেই **DHCP-তে সমস্যা**। IPv6-এর সমতুল্য = **FE80::/10 (link-local)**।
...
## Topic 2.9: "−2" কেন? ⭐⭐⭐ (এটা বুঝলে Subnetting পুরো পরিষ্কার)

### সমস্যা
`192.168.1.0/24` → Host bit = 8 → `2⁸ = 256` টা ঠিকানা।
কিন্তু ব্যবহারযোগ্য মাত্র **254**। বাকি দুটো গেল কোথায়?

### উত্তর — দুটো ঠিকানা সংরক্ষিত
...
### 🔹 প্রথমটা কেন দেওয়া যায় না?
`192.168.1.0` **পুরো network-টাকেই বোঝায়** — এটা network-এর নাম।

> *"এই পুরো network-এর পরিচয় হলো 192.168.1.0"*

Router-এর routing table-এ এই ঠিকানাই লেখা থাকে। কোনো একক PC-কে এটা দেওয়া যায় না — তাহলে "network" আর "একটা কম্পিউটার" আলাদা করা যেত না।

### 🔹 শেষেরটা কেন দেওয়া যায় না?
`192.168.1.255` হলো **Broadcast Address**।

এখানে data পাঠালে **network-এর সব device** সেটা পাবে। এটা একটা "সবাইকে ডাকার" ঠিকানা — কোনো নির্দিষ্ট PC-র নিজস্ব ঠিকানা হতে পারে না।

### 🔹 তাই
```
মোট ঠিকানা          = 256
সংরক্ষিত            =   2      (1টা Network + 1টা Broadcast)
                     ─────
ব্যবহারযোগ্য Host   = 254  ✅
```

⭐ **এই যুক্তি সব subnet-এ খাটে** — যেকোনো block-এর **প্রথমটা Network**, **শেষটা Broadcast**।

---

## Topic 3: Subnet Mask Cheat Sheet ⭐⭐⭐ (মুখস্থ করে ফেলো)

### ⚡ ৫ সেকেন্ডের Exam Trick ⭐⭐⭐

টেবিল মনে না থাকলেও এই তিন লাইনেই যেকোনো প্রশ্ন সমাধান —
...
```
Total :  256 → 128 → 64 → 32 → 16 → 8 → 4     (প্রতিবার অর্ধেক)
Usable:  254 → 126 → 62 → 30 → 14 → 6 → 2     (প্রতিটি Total − 2)
```

⚠️ **/31 ও /32 ব্যতিক্রম** — /31-এ usable 0 (তবু router-router link-এ বিশেষ নিয়মে ব্যবহৃত), /32 = একটামাত্র host (loopback বা নির্দিষ্ট route)

### CIDR আসলে কী — Binary-তে দেখো ⭐

> **/n মানে বাম দিক থেকে টানা `n` সংখ্যক 1।** বাকিটা সব 0।
> 1 = Network অংশ · 0 = Host অংশ
...
### Subnetting-এ যেগুলো লাগবে (শেষ octet)

| CIDR | শেষ Octet (binary) | Decimal Mask |
|---|---|---|
...
⭐ **লক্ষ্য করো:** 1-গুলো সবসময় **বাম দিক থেকে টানা**, মাঝে কখনো ফাঁক থাকে না।

### 🔒 বৈধ Mask Octet — মাত্র ৯টি ⭐⭐⭐

| Binary | Decimal | কয়টা 1 |
|---|---|---|
| `00000000` | **0** | 0 |
| `10000000` | **128** | 1 |
| `11000000` | **192** | 2 |
...
> **এগুলোর বাইরে আর কিছু হতে পারে না।**

**কেন?** একটা octet-এ 8টা bit। 1-গুলো টানা বাম থেকে বসতে হবে, তাই সম্ভাবনা মাত্র **৯টি** — 0টা 1, 1টা 1, 2টা 1, … 8টা 1। ব্যস।

⭐ **সংখ্যাগুলোর ধারা:** প্রতিবার **আগেরটার সাথে পরের ঘর মান যোগ** হচ্ছে
```
0 → +128 → 128 → +64 → 192 → +32 → 224 → +16 → 240
  → +8 → 248 → +4 → 252 → +2 → 254 → +1 → 255
```

### ⚠️ MCQ ফাঁদ
এগুলো **কখনোই বৈধ subnet mask নয়** —
| ভুল Mask | কেন |
|---|---|
| 255.255.255.**100** | `100 = 01100100` — শুরুতেই 0, টানা নয় ❌ |
| 255.**0**.255.0 | মাঝে 0 এসে আবার 255 — ফাঁক আছে ❌ |
| 255.255.**191**.0 | `191 = 10111111` — টানা নয় ❌ |
| 255.255.255.**200** | `200 = 11001000` — মাঝে ফাঁক ❌ |

**পরীক্ষার কৌশল:** অপশনে যে সংখ্যাটা উপরের ৯টার তালিকায় নেই — সেটাই ভুল উত্তর। আলাদা করে binary করার দরকার নেই।
...
মোট 1 = 8 + 8 + 4 = 20   →   /20  ✅
Host bit = 32 − 20 = 12   →   Host = 2¹² − 2 = 4094
```
...
## Topic 2.5: ARP ও RARP বিস্তারিত ⭐⭐⭐

### ARP (Address Resolution Protocol)

**Definition (English):** **ARP** resolves a known **IP address** into the corresponding **MAC address** within a local network.

> ### **ARP = IP → MAC**
> প্রশ্নটা হলো: *"এই IP Address-এর MAC Address কী?"*

### কেন দরকার?
Router IP দেখে ঠিক **network** খুঁজে দেয়। কিন্তু ঐ network-এর ভিতরে **কোন device**-এ পাঠাবে, সেটা ঠিক করতে **MAC address** লাগে (Layer 2)।
IP জানা আছে, MAC জানা নেই — এই ফাঁকটাই ARP পূরণ করে।

### 🔍 কীভাবে কাজ করে

Laptop A জানতে চায় `192.168.1.20`-এর MAC কী। সে পুরো local network-এ **broadcast** করে:
...
**সবাই শোনে।** কিন্তু শুধু **Laptop B** জবাব দেয় —
...
এখন Laptop A MAC জেনে গেল, data পাঠাতে পারবে।

### ARP Flow
```
   Laptop A                    Laptop B
      │
      │  জানা: IP = 192.168.1.20
      │  অজানা: MAC = ?
      │
...
⭐ **Request = Broadcast** (সবার কাছে) · **Reply = Unicast** (শুধু যে চেয়েছিল তার কাছে)
⭐ উত্তরটা **ARP Cache**-এ জমা থাকে, যাতে বারবার জিজ্ঞেস করতে না হয় (`arp -a` দিয়ে দেখা যায়)

---

### RARP (Reverse Address Resolution Protocol)

**Definition (English):** **RARP** does the opposite — a device that knows only its **MAC address** asks a server for its **IP address**.

> ### **RARP = MAC → IP**

### কখন লাগত?
পুরনো **Diskless Computer** চালু হলে সে শুধু নিজের MAC জানত (NIC-এ পোড়ানো থাকে), কিন্তু IP জানত না — কারণ hard disk নেই, কোথাও সংরক্ষিত ছিল না।

```
   Diskless PC                    RARP Server
      │
      │  জানা: MAC = AA-AA-AA-AA-AA-AA
      │  অজানা: IP = ?
      │
      │  ── "এই MAC-এর IP কত?" ──►
      │
...
### কেন এখন ব্যবহার হয় না? ⭐
**DHCP** এই কাজটা অনেক ভালোভাবে করে — কম্পিউটার চালু হলে DHCP server নিজেই IP, subnet mask, gateway, DNS — সবকিছু দিয়ে দেয়। RARP শুধু IP দিতে পারত, আর প্রতিটি subnet-এ আলাদা server লাগত।
তাই **RARP এখন প্রায় অচল** — বইয়ে আছে, বাস্তবে নেই।
...
| দিক | **IP → MAC** | **MAC → IP** |
| জানা আছে | IP | MAC |
| প্রশ্ন | "এই IP কার?" | "আমার IP কত?" |
| Layer | 2–3 এর মাঝে | 2–3 এর মাঝে |
| ব্যবহার | ✅ **এখনও ব্যবহৃত** | ❌ **অচল** (DHCP নিয়েছে) |

**সম্পর্কিত:** **Proxy ARP** (Router অন্যের হয়ে জবাব দেয়) · **Gratuitous ARP** (নিজের IP ঘোষণা করা, duplicate ধরতে)

### 🔁 Revision Box
**ARP = IP → MAC**, Request **broadcast**, Reply **unicast**, উত্তর **ARP cache**-এ জমে। **RARP = MAC → IP**, diskless computer-এর জন্য ছিল, **এখন DHCP নিয়ে নিয়েছে**। মনে রাখো: **A**RP-এ **A**ddress (IP) দিয়ে হার্ডওয়্যার খোঁজো, **R**ARP-এ **R**everse।
